...
class FileView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        for j in raees["bins"]:
            solver.Add(
                sum(x[(i, j)] * raees["weights"][i] for i in raees["items"])
                <= y[j] * raees["bin_capacity"]
            )
        ...
        if status == pywraplp.Solver.OPTIMAL:
            ...
            context = {"message": "The problem does not have an optimal solution."}
